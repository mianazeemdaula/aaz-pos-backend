...
export const getDailyReportPDF = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        // ── Sales table ──
        if (regularSales.length > 0) {
            sectionHeader(`Sales (${regularSales.length})`, "#166534");
            doc.x = doc.page.margins.left;
            doc.fontSize(8);
            const t = doc.table({
                columnStyles: [30, 60, "*", 60, 60, 70, 70],
        ...
        if (saleReturns.length > 0) {
            sectionHeader(`Sales Returns (${saleReturns.length})`, "#b91c1c");
            doc.x = doc.page.margins.left;
            doc.fontSize(8);
            const t = doc.table({
                columnStyles: [30, 60, "*", 70, 70, 70, 80],
                rowStyles: (row: number) => row === 0 ? { backgroundColor: "#fee2e2", fontStyle: "bold", fontSize: 9 } : {},
            });
            t.row(["#", "Time", "Customer", "Refunded", "Tax Refund", "Total Return", "Orig. Invoice #"].map(h => ({ text: h, align: { x: "center", y: "center" } })));
            saleReturns.forEach((s, i) => {
                t.row([
                    { text: String(i + 1), align: { x: "center", y: "center" } },
                    { text: fmtDate(s.createdAt, "hh:mm A"), align: { x: "center", y: "center" } },
                    { text: s.customer?.name ?? "Walk-in", align: { x: "left", y: "center" } },
                    { text: fmtCurrency(Math.abs(s.paidAmount)), align: { x: "right", y: "center" } },
                    { text: fmtCurrency(Math.abs(s.taxAmount)), align: { x: "right", y: "center" } },
                    { text: fmtCurrency(Math.abs(s.totalAmount)), align: { x: "right", y: "center" } },
                    { text: s.parentSaleId ? `#${s.parentSaleId}` : "N/A", align: { x: "center", y: "center" } },
                ]);
            });
            t.row([
                { text: "Total", colSpan: 3, align: { x: "right", y: "center" } },
                { text: fmtCurrency(saleReturns.reduce((s, x) => s + Math.abs(x.paidAmount), 0)), align: { x: "right", y: "center" } },
                { text: fmtCurrency(saleReturns.reduce((s, x) => s + Math.abs(x.taxAmount), 0)), align: { x: "right", y: "center" } },
                { text: fmtCurrency(totalSaleReturns), align: { x: "right", y: "center" } },
